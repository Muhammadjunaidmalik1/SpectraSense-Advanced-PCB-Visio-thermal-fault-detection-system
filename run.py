...
# ==========================================
# Script with the final optimizations applied and logics corrected. Without FastAPI interface with frontend.
# ==========================================
...
# --- LOGIC THRESHOLDS ---
RECORD_MIN_DURATION = 10.0  # Minimum recording length
WAIT_FOR_TRIGGER = 1.0      # Seconds of contact to start recording
TEMP_THRESHOLD = 250.0      # Below this = Blue / Low Temp Warning
OVER_TEMP_LIMIT = 300.0     # Above this = Red / Over Temp Alarm
...
def main():
    ...
    print("\n--- SYSTEM READY ---")
    print(f"Trigger: Contact > {WAIT_FOR_TRIGGER}s (Starts regardless of Temp)")
        ...
        while True:
            ...
            if not is_recording:
                ...
                if intersecting:
                    ...
                    status_text = f"Status: CONTACT ({elapsed_contact:.1f}s)"

                    # WARNING: Low Temp (Info Only)
                    if max_temp_val < TEMP_THRESHOLD:
                        if int(time.time() * 4) % 2 == 0: # Flash Orange
                             cv2.putText(display_image, "WARNING: LOW TEMP", (50, 400), 
                                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 0), 3) # Blue-ish warning color in BGR

                    # TRIGGER (Time only - removed temp condition)
                    if elapsed_contact >= WAIT_FOR_TRIGGER:
                        is_recording = True
                ...
                if max_temp_val < TEMP_THRESHOLD:
                     if int(time.time() * 2) % 2 == 0:
                            cv2.putText(display_image, "LOW TEMP DETECTED!", (50, 400), 
                                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 0), 3) # Blue/Orange
            ...
            # --- NEW COLOR SCHEME (BGR) ---
            if max_temp_val < 250.0:
                bar_color = (255, 0, 0) # Blue
            elif max_temp_val <= 300.0:
