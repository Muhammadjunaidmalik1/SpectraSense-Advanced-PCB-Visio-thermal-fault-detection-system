...
# RECORD_FPS = 10.0      # Set this lower to match actual processing speed
# SKIP_FRAMES = 3        # Run YOLO only every N frames to reduce lag
...
#         self.capture.set(cv2.CAP_PROP_BUFFERSIZE, 1) # Keep buffer small
...
#             # Minimal sleep to prevent thread hogging
#             time.sleep(0.001)
...
#     try:
#         # Keep 1080p for Alignment Matrix compatibility
...
#     last_results_boxes = [] # Cache detections
    
#     print("\n--- SYSTEM READY ---")
#     print(f"Recording FPS set to: {RECORD_FPS}")
#     print(f"YOLO running every {SKIP_FRAMES} frames")
...
#                 last_results_boxes = results[0].boxes # Update cache
#             else:
#                 # Reuse the result from the previous frame to save CPU
#                 # (Boxes might lag slightly behind fast movement, but fps increases huge)
#                 last_annotated_crop = last_annotated_crop # Just for clarity
...
#             # 3. Check Intersection (Using CACHED boxes)
#             list_irons = []
#             list_chips = []
...
#                         msg = f"Heat: {max_temp_val:.0f}/{TEMP_THRESHOLD}C"
#                         col = (0, 255, 255)
#                     else:
#                         msg = f"Hold: {max(0, WAIT_FOR_TRIGGER - elapsed_contact):.1f}s"
#                         col = (0, 255, 0)
                    
#                     cv2.putText(display_image, msg, (10, 600), cv2.FONT_HERSHEY_SIMPLEX, 0.7, col, 2)
...
#                     status_text = f"REC (Lock): {int(RECORD_MIN_DURATION - elapsed_rec_time)}s"
#                     status_col = (0, 0, 255)
#                 else:
#                     if intersecting:
...
#                         status_text = f"REC (Over): {int(elapsed_rec_time)}s"
#                         status_col = (0, 0, 255)
#                     else:
#                         print(f"--- STOPPED. Duration: {elapsed_rec_time:.1f}s ---")
#                         is_recording = False
...
#                 cv2.putText(display_image, status_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, status_col, 2)
#                 cv2.putText(display_image, f"Temp: {max_temp_val:.1f}C", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, status_col, 2)
...
def run_detection_logic():
    ...
    print("\n--- SYSTEM READY ---")
    print(f"Streaming to http://localhost:8000/video_feed")
...
@app.get("/")
def index():
    return {"status": "running", "stream": "/video_feed"}
